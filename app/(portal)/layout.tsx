...
import { getCustomerChannel } from "@/lib/chat/repository";

export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  const user = await requireSession();
  const channel = getCustomerChannel(user.customerId, user);
  const channelHref = channel ? `/chat/${channel.id}` : null;

  return (
    <ChatRealtimeProvider viewerId={user.id}>
      <div className="min-h-dvh lg:grid lg:grid-cols-[260px_1fr]">
        ...
        <aside className="hidden lg:flex sticky top-0 h-dvh flex-col border-r border-border bg-card">
          ...
          <div className="flex-1 overflow-y-auto py-2">
            <SidebarNav channelHref={channelHref} />
          </div>
        ...
        <div className="lg:hidden border-b border-border bg-card py-2">
          <SidebarNav orientation="horizontal" channelHref={channelHref} />
        </div>
