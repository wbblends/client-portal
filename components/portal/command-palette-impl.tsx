...
export function CommandPalette({
  ...
  /** Open immediately on mount. Set by the lazy wrapper when the palette is
   *  mounted in response to the ⌘K / trigger that should also open it. */
  startOpen?: boolean;
}) {
                ...
                {filtered.length === 0 ? (
                  ...
                  grouped.map((group, gi) => (
                    <div key={group.label} className={cn(gi > 0 && "mt-1")}>
                      <div className="px-4 pb-1 pt-2 text-[10px] font-bold uppercase tracking-[0.1em] text-muted-soft">
                        {group.label}
                      </div>
                      <ul role="listbox">
